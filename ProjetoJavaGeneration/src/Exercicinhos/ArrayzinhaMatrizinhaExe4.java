...
public class ArrayzinhaMatrizinhaExe4 {
	public static void main(String args[]) {
		...
		//não dá pra usar array vazia, para de ser doida
		...
		System.out.println("Atenções, leia o número de uma constante: ");
		constante = ler.nextFloat();

		System.out.println("\nMenu de las opçionxs");
		...
		do {
			if (op == 5) {
				System.out.println("Valeu de coração por utilizar o nosso sistemitcho, tmj !");
			} else {
				switch (op) {
				case 1:
					for (int i = 0; i < 2; i++) {
				...
				case 2:
					for (int i = 0; i < 2; i++) {
						for (int j = 0; j < 2; j++) {
							System.out.println("\nSubtraçãozinha: " + matrizSubtrai[i][j]);
						}
					}
					break;
				case 3:
				...
				default:
					System.out.println("Tem alguma coisa errada que tu colocou aí...");
	...
	//rever anotações do caderno - texto vermelho - cobrinha ^^^^^^^^^^^^
}
